...
export default function Model(props) {
    ...
    const { nodes } = useGLTF('/medias/Logo_1.glb');
    ...
    useFrame(() => {
        ...
        if (groupRef.current) {
            ...
            groupRef.current.position.x += (mousePosition.x - groupRef.current.position.x) * 0.1;
            groupRef.current.position.y += (mousePosition.y - groupRef.current.position.y) * 0.1;
        }
    });
...
useGLTF.preload('/medias/Logo_1.glb');
